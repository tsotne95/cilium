...
func (s *SharedStore) Reference(key SharedPolicyKey, owner uint16, now time.Time) (uint32, *SharedMetadata) {
	...
	if h, ok := s.keyToHandle[key]; ok {
		...
		meta.LastUsed = now
		metrics.PolicySharedMapOps.WithLabelValues("lookup", metrics.LabelValueOutcomeSuccess).Inc()
		return h, meta
	}

	metrics.PolicySharedMapOps.WithLabelValues("lookup", metrics.LabelValueOutcomeFail).Inc()
	...
	metrics.PolicySharedMapEntries.WithLabelValues(metrics.LabelSharedMapOne).Inc()
	metrics.PolicySharedMapOps.WithLabelValues("add", metrics.LabelValueOutcomeSuccess).Inc()
	return handle, meta
}
...
func (s *SharedStore) Dereference(handle uint32, owner uint16) (int, bool, SharedPolicyKey) {
	...
	return meta.RefCount, meta.RefCount == 0, meta.Key
}
...
func (s *SharedStore) GarbageCollect(cutoff time.Time) []uint32 {
	...
	for handle, meta := range s.handleToEntry {
		if meta.RefCount == 0 && meta.LastUsed.Before(cutoff) {
			...
			delete(s.keyToHandle, meta.Key)
			metrics.PolicySharedMapEntries.WithLabelValues(metrics.LabelSharedMapOne).Dec()
			metrics.PolicySharedMapOps.WithLabelValues("delete", metrics.LabelValueOutcomeSuccess).Inc()
		}
	}
	return deleted
}
