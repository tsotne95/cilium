...
func TestPolicySharedMapMetrics(t *testing.T) {
	...
	_, err := SyncEndpointOverlay(999, seq)
	...
	// 3. Trigger GC
	mgr.store.GarbageCollect(time.Now().Add(time.Hour))

	// Verify Ops: 0 Delete (No store entries to GC)
	require.Equal(t, float64(0), opsDelete.Get()-initialDeletes, "GC should have nothing to delete")
}
